...
impl LayerKvCache {
    pub fn new(context: &MetalContext, heads: usize, max: usize, dim: usize) -> Result<Self> {
        ...
        Ok(Self {
            key: Tensor::zeros(context, &shape)?,
            value: Tensor::zeros(context, &shape)?,
            len: 0,
            max_seq_len: max,
            num_heads: heads,
            head_dim: dim,
    ...
    pub fn append(&mut self, context: &MetalContext, key: Tensor, value: Tensor) -> Result<()> {
        ...
        kv_cache_write_f32(context, &self.key, &key, self.len)?;
        kv_cache_write_f32(context, &self.value, &value, self.len)?;
        self.len += seq;
        Ok(())
    }
}
...
impl KvCache {
    pub fn new(
        ...
        Ok(Self {
            layers: (0..layers)
                .map(|_| LayerKvCache::new(context, heads, max, dim))
