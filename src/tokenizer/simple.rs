...
impl CharTokenizer {
    pub fn load_json(path: impl AsRef<Path>) -> Result<Self> {
        ...
        let file: TokenizerFile = serde_json::from_str(&text)
            .map_err(|error| TinyError::Tokenizer(format!("invalid tokenizer.json: {error}")))?;

        if file.tokenizer_type != "char" {
        ...
        let mut token_to_id = HashMap::with_capacity(file.vocab.len());

        for (index, token) in file.vocab.iter().enumerate() {
            if token.is_empty() {
                return Err(TinyError::Tokenizer(format!("token {index} is empty")));
            }

            if token_to_id.contains_key(token) {
                return Err(TinyError::Tokenizer(format!("duplicate token: {token:?}")));
            }

            let token_id = u32::try_from(index).map_err(|_| {
                TinyError::Tokenizer("vocabulary exceeds u32 token IDs".to_string())
            })?;

            token_to_id.insert(token.clone(), token_id);
        }

        validate_special_id("bos_token_id", file.bos_token_id, file.vocab.len())?;

        validate_special_id("eos_token_id", file.eos_token_id, file.vocab.len())?;

        validate_special_id("unk_token_id", file.unk_token_id, file.vocab.len())?;
        ...
        Ok(Self {
            ...
            bos_token_id: file.bos_token_id,

            eos_token_id: file.eos_token_id,

            unk_token_id: file.unk_token_id,
        })
    }
    ...
    pub fn decode(&self, token_ids: &[u32], skip_special_tokens: bool) -> Result<String> {
        ...
        for &token_id in token_ids {
            ...
            let token = self.id_to_token.get(index).ok_or_else(|| {
            ...
            if skip_special_tokens && self.is_special_token(token_id) {
                continue;
            }
...
impl Tokenizer for CharTokenizer {
    ...
    fn decode(&self, token_ids: &[u32], skip_special_tokens: bool) -> Result<String> {
        CharTokenizer::decode(self, token_ids, skip_special_tokens)
    }
...
fn validate_special_id(name: &str, token_id: Option<u32>, vocab_size: usize) -> Result<()> {
