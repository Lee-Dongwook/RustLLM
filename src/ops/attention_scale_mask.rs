...
pub fn attention_scale_mask_f32(
        ...
        || key_len == 0
    {
        return Err(
            TinyError::InvalidShape(
                "attention query/key length cannot be zero"
                    .to_string(),
            ),
        );
    }
    ...
    let query_end =
        query_start_pos
            ...
            .ok_or_else(|| {
                TinyError::InvalidShape(
                    "attention query position overflow"
                        .to_string(),
                )
            })?;
    ...
    if input.len() % (query_len * key_len)
        != 0
    {
        return Err(
            TinyError::InvalidShape(
    ...
    let shader_source =
        include_str!(
            "../../kernels/attention_scale_mask.metal"
        );
    ...
    let query_start_pos =
        ...
        .map_err(|_| {
            TinyError::InvalidShape(
                "query start position exceeds u32"
                    .to_string(),
            )
        })?;
    ...
    let numel =
        ...
        .map_err(|_| {
            TinyError::InvalidShape(
                "attention score size exceeds u32"
                    .to_string(),
            )
        })?;
