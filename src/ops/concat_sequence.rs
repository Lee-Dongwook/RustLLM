...
pub fn concat_sequence_f32(
        ...
        || right.rank() != 4
    {
        return Err(
            TinyError::ModelFormat(
                "concat_sequence expects rank-4 tensors"
                    .to_string(),
            ),
        );
    }
    ...
    let output_len = batch
        ...
        .ok_or_else(|| {
            TinyError::InvalidShape("concatenated tensor size overflow".to_string())
        })?;
    ...
    let left_seq = u32::try_from(left_seq).map_err(|_| {
        TinyError::InvalidShape("left sequence length exceeds u32".to_string())
    })?;
    let right_seq = u32::try_from(right_seq).map_err(|_| {
        TinyError::InvalidShape("right sequence length exceeds u32".to_string())
    })?;
    let num_heads = u32::try_from(num_heads).map_err(|_| {
        TinyError::InvalidShape("number of heads exceeds u32".to_string())
    })?;
    let head_dim = u32::try_from(head_dim).map_err(|_| {
        TinyError::InvalidShape("head dimension exceeds u32".to_string())
    })?;

    for (index, value) in [left_seq, right_seq, num_heads, head_dim].iter().enumerate() {
        encoder.set_bytes(
            (index + 3) as u64,
